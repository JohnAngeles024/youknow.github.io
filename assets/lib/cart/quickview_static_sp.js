$(document).ready(function(){
	var arrList = $('.main').find('.lineup__content__list__box');
	var arrPid = '';
	var flg = 0;
...
function SetPrice(p1,p2){
	...
	if(p1 !== null){
		if(parseInt(p1) == parseInt(p2)){
			html += priceSeparate(p1)+'円[税抜]';
		}else if (parseInt(p1) > parseInt(p2)){
			html += '<span class="preset_price_mini">'+priceSeparate(p1)+'<span class="smaller">円[税抜]</span></span>';
			html += priceSeparate(p2)+'円[税抜]';
